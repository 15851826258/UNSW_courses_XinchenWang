...
def viterbi_algorithm(State_File, Symbol_File, Query_File):  # do not change the heading of the function
    ...
    with open(Symbol_File, "r") as symbolFile:
        ...
        # Last additional column is for UNK
        B = np.zeros(shape=(numOfStates, numOfSymbols + 1))
        ...
        for j in range(1, queryLen):
            for i in range(numOfStates - 2):
                ...
                for k in range(numOfStates - 2):
                    if query[j] == "UNK":
                        opt.append(result[k][j - 1] * A[k][i] * B[i][-1])
                    else:
        ...
        for j in range(1, queryLen):
            for i in range(numOfStates - 2):
                ...
                for l in range(numOfStates - 2):
                    for n in range(k):
                        if query[j] == "UNK":
                            opt.append(result[l, j - 1, n] * A[l][i] * B[i][-1])
                        else:
...
def advanced_decoding(State_File, Symbol_File, Query_File):  # do not change the heading of the function
    ...
    with open(State_File, "r") as stateFile:
        ...
        # Using Absolute Discounting to smooth the probabilities
        for i in range(numOfStates):
            count = numOfStates
            rest = 1
            # Subtract an discount value from every count
            for j in range(numOfStates):
                if A[i, j] != 0:
                    A[i, j] = (A[i, j] - d) / sumStateMat[i]
                    count -= 1
                    rest -= A[i, j]

            # Evenly distribute the rest probabilities for unseen states from current state
            for j in range(numOfStates):
                if A[i, j] == 0:
                    A[i, j] = rest / count
    ...
    with open(Symbol_File, "r") as symbolFile:
        ...
        # Last additional column is for UNK
        B = np.zeros(shape=(numOfStates - 2, numOfSymbols + 1))
        ...
        for i in range(numOfStates - 2):
            count = numOfSymbols + 1
            rest = 1
            # Subtract an discount value from every count
            for j in range(numOfSymbols + 1):
                if B[i][j] != 0:
                    B[i][j] = (B[i][j] - d)/sumSymbolMat[i]
                    count -= 1
                    rest -= B[i][j]
